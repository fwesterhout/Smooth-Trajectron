...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(self,
        ...
        !!! Look at this output parameter, might indicate whether attention is used !!!
        ...
        # Encode History #
        ##################
        node_history_encoded, node_outputs = self.encode_node_history(mode,
                                                        node_history_st,
                                                        first_history_indices)
        ...
        if self.hyperparams['edge_encoding']:
            ...
            for edge_type in self.edge_types:
                ...
                encoded_edges_type, last_index_per_sequence, outputs = self.encode_edge(mode,
                                                      node_history,
                                                      node_history_st,
                                                      edge_type,
                                                      neighbors[edge_type],
                ...
                last_index_list.append(last_index_per_sequence)
            ...
            # Encode Node Edges #
            #####################
            total_edge_influence,attn = self.encode_total_edge_influence(mode,
                                                                    node_edges_encoded,
            ...
            TV = self.total_variation_loss(attn)
        ...
        return x, x_r_t, y_e, y_r, y, n_s_t0, TV # !!! extra output here to get eventually to loss function in training_loss function
    ...
    def encode_node_history(self, mode, node_hist, first_history_indices):
        ...
        return outputs[torch.arange(first_history_indices.shape[0]), last_index_per_sequence], outputs
    ...
    def encode_edge(self,
        ...
        if self.hyperparams['dynamic_edges'] == 'yes':
            return ret * combined_edge_masks, last_index_per_sequence, outputs
        else:
            return ret, last_index_per_sequence, outputs

    def encode_total_edge_influence(self, mode, encoded_edges, node_history_encoder, batch_size, last_index_list, output_list, node_outputs):
...
# !!! Here you get the attention probs of the Additive Attention module in the _ output !!!     


        elif self.hyperparams['edge_influence_combine_method'] == 'attention':
            
            #print('edge influence combine method is attention')
            #print("")
            ...
            if len(encoded_edges) == 0:
                ...
                encoded_edges = torch.stack(encoded_edges, dim=1) #makes list into tensor
                
                outputs = torch.stack(output_list, dim=1)
                
                last_index = torch.stack(last_index_list, dim=1)
                 
                    
                n_ts = outputs.shape[2]-1
                
                attn = torch.zeros((outputs.shape[0],outputs.shape[1],n_ts,1), device = self.device)
                
                for tidx in range(n_ts):
                    tidx_new = last_index-n_ts+tidx+1 #shape [bs,num_edge_types]
                    no_input = tidx_new <= -n_ts-1
                    tidx_new[no_input] = -n_ts
                    
                    batch_idx, type_idx = torch.meshgrid((torch.arange(outputs.shape[0],device=self.device),
                                                          torch.arange(outputs.shape[1],device=self.device)))
                    output_tidx = outputs[batch_idx,type_idx,tidx_new] #shape [bs,num_edge_types, dim_internal_lstm]
                    
                    node_output_tidx = node_outputs[batch_idx[:,0],tidx_new[:,0]]
                    
                    _ , attn_tidx = self.node_modules[self.node_type + '/edge_influence_encoder'](output_tidx,
                                                                                                  node_output_tidx)
                ...
                combined_edges, attn_probs = self.node_modules[self.node_type + '/edge_influence_encoder'](encoded_edges,
        ...
        return combined_edges, attn
    

    
    def total_variation_loss(self, attn):
        """
        ...
        """
        attn_dif_sq = (attn[:,:,1:] -attn[:,:,:-1])**2    

        attn_MSE = torch.sum(attn_dif_sq,dim=1)
        attn_RMSE = torch.sqrt(attn_MSE+1e-7) #to avoid 0 and gradient exploding
        attn_sum_tidx = torch.sum(attn_RMSE,dim=1)
        
        attn_loss = torch.mean(attn_sum_tidx)
    ...
    def encoder(self, mode, x, y_e, num_samples=None):
        ...
        :return: tuple(z, kl_obj)
            ...
            - kl_obj: KL Divergence between q and p
        """
    ...
    def train_loss(self,
        ...
        x, x_nr_t, y_e, y_r, y, n_s_t0, TV = self.obtain_encoded_tensors(mode=mode,
                                                                     inputs=inputs,
                                                                     inputs_st=inputs_st,
                                                                     labels=labels,
                                                                     labels_st=labels_st,
        ...
        # Set scaling factor beta
        beta = self.hyperparams['beta']

        # Add Total variation loss to objective function
        ELBO = log_likelihood - self.kl_weight * kl + 1. * mutual_inf_p + beta * TV
        loss = -ELBO
    ...
    def predict(self,
        ...
        x, x_nr_t, _, y_r, _, n_s_t0, TV = self.obtain_encoded_tensors(mode=mode,
                                                                   inputs=inputs,
                                                                   inputs_st=inputs_st,
                                                                   labels=None,
                                                                   labels_st=None,
