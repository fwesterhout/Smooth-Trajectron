...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(self,
        ...
        return x, x_r_t, y_e, y_r, y, n_s_t0, TV
    ...
    def encode_node_history(self, mode, node_hist, first_history_indices):
        ...
        return outputs[torch.arange(first_history_indices.shape[0]), last_index_per_sequence.long()], outputs
            ...
            if len(encoded_edges) == 0:
                ...
                for tidx in range(n_ts):
                    ...
                    _ , attn_tidx = self.node_modules[self.node_type + '/edge_influence_encoder'](output_tidx,
                                                                                                  node_output_tidx)
                    #attn_tidx[no_input] = attn_tidx[no_input] - 3.0
                    attn[:,:,tidx] = attn_tidx
    ...
    def total_variation_loss(self, attn):
        ...
        Returns
        -------
        Attention loss to be added to total loss function 

        """
        attn_dif_sq = (attn[:,:,1:] -attn[:,:,:-1])**2    
        #attn_dif_sq[attn_dif_sq>=4] = attn_dif_sq[attn_dif_sq>=4] - attn_dif_sq[attn_dif_sq>=4] #to avoid getting loss when going from no input to input data
        ...
        # print("attn_dif_sq :", attn_dif_sq)
        # print("attn_MSE :", attn_MSE)
        # print("attn_RMSE :", attn_RMSE)
        # print("attn_sum_tidx:", attn_sum_tidx)
    ...
    def train_loss(self,
        ...
        beta = self.hyperparams['beta'] #beta is scaling factor for smooth attention
        print("beta = ",beta)

        ELBO = log_likelihood - self.kl_weight * kl + 1. * mutual_inf_p + beta * TV # !!! updated loss function with smooth-term
        loss = -ELBO
